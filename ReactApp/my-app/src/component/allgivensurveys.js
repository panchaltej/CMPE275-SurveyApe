...
// import {removemenu} from '../actions/index';
 ...
 import {allgivensurveys} from '../action/getallsavedsurveys'

class allgiven extends Component{
    state={
        usreid:1,
        current_survey:{},
        dashboard:true
    ...
    componentDidMount()
    {
        ...
        API.allgivensurveys(payload).then
        ((output) => {
            console.log("In dash board:",output);
            this.props.allgivensurveys(output)
             }) ; 
    }

    fetch_all_open_unique_surveys()
    {
        if(this.props.getallgivensurveys.length > 0){
            return this.props.getallgivensurveys.map((surveys,index) =>{
                return(
  ...
  render()
    {
      ...
      <div class="d-flex align-items-center p-3 my-3 text-white-50 bg-purple rounded box-shadow">
        ...
        <div class="lh-100">
          <h6 class="mb-0 text-white lh-100">All given surveys</h6>
...
function mapDispatchToProps(dispatch) {
    return {
        allgivensurveys : (data) => dispatch(allgivensurveys(data))
    };
  }

  function mapStateToProps(state){
    return {
        getallgivensurveys: state.getallgivensurveys
    }
}

export default withRouter(connect(mapStateToProps, mapDispatchToProps)(allgiven));